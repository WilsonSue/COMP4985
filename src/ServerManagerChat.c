#include "../include/ServerManagerChat.h"
...
{
    uint8_t  version = 1;
    uint16_t size    = htons((uint16_t)(strlen(message)));
    ...
    size_t total_size = offset + ntohs(size);
...
void addClient(int socket)
{
    ...
    if(found)
    {
        ...
        snprintf(msg, sizeof(msg), "/d %d", countActiveClients());
...
void notifyServerManagers(const char *message)
{
    ...
    for(int i = 0; i < MAX_CLIENTS; i++)
    {
        if(clients[i].is_active && clients[i].is_server_manager)
        {
            ...
            snprintf(formattedMsg, sizeof(formattedMsg), "/d %d", countActiveClients());
            send_message_protocol(clients[i].socket, formattedMsg);
        }
    }
    pthread_mutex_unlock(&clients_mutex);
}
