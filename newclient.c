    ...
    static void write_to_socket(int sockfd, const char *message) {
        ...
        write(sockfd, &size, sizeof(uint16_t)); // Write the size of the command in network byte order
    ...
    static int read_from_socket(int sockfd) {
        ...
        size = ntohs(size); // Convert size from network byte order to host byte order
